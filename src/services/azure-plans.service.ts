...
export async function getAutomationMetricsForPlans(testPlans: TestPlan[]): Promise<AutomationMetricsResponse> {
  ...
  for (const item of batchRes.data.value) {
    ...
    const status = item.fields[ADO_AUTOMATION_STATUS_FIELD!];
    const category = item.fields[ADO_TESTING_TYPE_FIELD!] || 'Uncategorized';
    const tool = item.fields[ADO_AUTOMATION_TOOLS_FIELD!] || 'Unknown';
    const link = `https://dev.azure.com/${process.env.ADO_ORGANIZATION}/${ADO_PROJECT}/_workitems/edit/${id}`;

    // update overall category/tool
    if (!overallCategoryMap.has(category)) overallCategoryMap.set(category, { manual: 0, automated: 0 });
    const cat = overallCategoryMap.get(category)!;
    if (status === 'Automated') { overallAutomated++; overallLinks.automated.push(link); cat.automated++; }
    else { overallManual++; overallLinks.manual.push(link); cat.manual++; }
    overallToolMap.set(tool, (overallToolMap.get(tool) || 0) + 1);

    // assign to each plan this testCase belongs to
    for (const planId of testCaseToPlans.get(id)!) {
      if (!metricsMap.has(planId)) metricsMap.set(planId, emptyPlan(planId, planNamesMap.get(planId)!));
      const pm = metricsMap.get(planId)!;
      if (status === 'Automated') { pm.automated++; pm.links.automated.push(link); }
      else { pm.manual++; pm.links.manual.push(link); }
      pm.categories = upsertCategory(pm.categories, category, status);
      pm.tools = upsertTool(pm.tools, tool);
    }
  }

  // Build plans array with passRate
  ...
  await Promise.all(Array.from(metricsMap.values()).map(async pm => {
    ...
    // calculate passRate via test points (local array)
    const results: string[] = [];
    const suitesRes = await azureClient.get(`/${ADO_PROJECT}/_apis/testplan/plans/${pm.planId}/suites?api-version=${AZURE_API_VERSION}`);
    await Promise.all(suitesRes.data.value.map(async (suite: any) => {
      const pointsRes = await azureClient.get(`/${ADO_PROJECT}/_apis/test/plans/${pm.planId}/suites/${suite.id}/points?api-version=${AZURE_API_VERSION}`);
